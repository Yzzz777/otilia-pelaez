...
const APP={
  ...
  accounts:{
    ...
    secretaria:{email:'secretariaoti@gmail.com',password:'SECRETOTI@2025',role:'secretaria',name:'Secretaría'},
    directora:{email:'directoraoti@gmail.com',password:'DIROTILAEZ@2025',role:'directora',name:'Directora'},
    orientacion:{email:'orientacionoti@gmail.com',password:'ORIENTOTI@2025',role:'orientacion',name:'Orientación'},
    deporte:{email:'deporteoti@gmail.com',password:'DEPORTOTI@2025',role:'deporte',name:'Ed. Física / Deporte'}
  }
};
...
function ensureDefaultAccounts(){
  ...
  Object.keys(defaults).forEach(function(k){ APP.accounts[k]=defaults[k]; });
}
...
function checkRegTipo(){
  ...
  var padreSection=document.getElementById('reg-padre-section');
  var alumnoSection=document.getElementById('reg-alumno-section');
  if(tipo==='estudiante'){
    if(padreSection) padreSection.style.display='none';
    if(alumnoSection) alumnoSection.style.display='block';
    if(typeof checkSecundaria==='function') checkSecundaria();
  } else {
    if(padreSection) padreSection.style.display='block';
    if(alumnoSection) alumnoSection.style.display='none';
  }
}
...
function loginAs(user){
  ...
  var mobMenu=document.getElementById('mobile-menu');if(mobMenu)mobMenu.style.display='none';
  document.body.classList.add('logged-in');
  document.body.classList.remove('public-mode');
  document.getElementById('nav-username').textContent=user.name;
  const roles={admin:'ADMIN',profesor:'PROF',estudiante:'EST',padre:'PADRE',enfermeria:'ENFER',secretaria:'SECRET',directora:'DIRECT',orientacion:'ORIENT',deporte:'DEPORTE'};
  ...
  showPage('home');
  buildPortalButtons(user.role);
  var pbs=document.getElementById('portal-buttons-section');if(pbs)pbs.style.display='block';
  ...
  else if(user.role==='secretaria'){showPage('secretaria');}
  else if(user.role==='directora'){showPage('directora');}
  else if(user.role==='orientacion'){showPage('orientacion');}
  else if(user.role==='deporte'){showPage('deporte');}
  ...
  // Show comunicados + check birthdays after login
  setTimeout(function(){
    var cs=document.getElementById('comunicados-home-section');if(cs)cs.style.display='block';
...
function showPortalFab(role){
  ...
  if(!fab)return;
  var labels={admin:'Panel Admin',profesor:'Mi Panel',estudiante:'Mi Portal',padre:'Mi Portal',enfermeria:'Enfermería',secretaria:'Secretaría',directora:'Dirección',orientacion:'Orientación',deporte:'Deportes'};
  var icons={admin:'⚙️',profesor:'👨‍🏫',estudiante:'🎓',padre:'👪',enfermeria:'🏥',secretaria:'🗂️',directora:'👩‍💼',orientacion:'🧭',deporte:'⚽'};
...
function goToPortal(){
  ...
  else if(role==='enfermeria') showPage('enfermeria');
  else if(role==='secretaria') showPage('secretaria');
  else if(role==='directora') showPage('directora');
  else if(role==='orientacion') showPage('orientacion');
  else if(role==='deporte') showPage('deporte');
}
...
function logout(){
  ...
  closeSidebar();
  var pbs=document.getElementById('portal-buttons-section');if(pbs)pbs.style.display='none';
  showChatWidget(false);
  showAllBotFabs(false);
  ...
  var bday=document.getElementById('birthday-banner');if(bday)bday.style.display='none';
  var comSec=document.getElementById('comunicados-home-section');if(comSec)comSec.style.display='none';
...
function renderStudentTable(){
  ...
  if(!tbody)return;
  tbody.innerHTML=APP.students.map(s=>`<tr><td><strong>${s.nombre} ${s.apellido}</strong></td><td>${s.grado}</td><td><code style="background:#f0f0f0;padding:2px 6px;border-radius:4px;">${s.id}</code></td><td>${s.email||'—'}</td><td>${s.carrera}</td><td style="display:flex;gap:4px;flex-wrap:wrap;"><button class="tbl-btn" onclick="openNotaTrimestre('${s.id}')" title="Notas por trimestre">📝</button><button class="tbl-btn" onclick="openBoletin('${s.id}')" title="Boletín">🖨️</button><button class="tbl-btn del" onclick="deleteStudent('${s.id}')">🗑</button></td></tr>`).join('')||'<tr><td colspan="6" style="color:#888;text-align:center;padding:16px;">Sin estudiantes</td></tr>';
  updateCounters();
}
...
function renderRoleHomeSection(){
  ...
  var name=APP.currentUser?APP.currentUser.name:'';
  var wrap=document.getElementById('home-role-section');
  if(!wrap||!APP.currentUser) return;

  var roleColors={
    admin:'linear-gradient(135deg,#1a2a50,#0d1f3e)',
    profesor:'linear-gradient(135deg,#0d2a1f,#0a3d2e)',
    estudiante:'linear-gradient(135deg,#1a3a5c,#0d2440)',
    padre:'linear-gradient(135deg,#2a1a50,#1a0d3e)',
    enfermeria:'linear-gradient(135deg,#0d3030,#062020)',
    secretaria:'linear-gradient(135deg,#2c3e7a,#16213e)',
    directora:'linear-gradient(135deg,#5c2e7a,#2d1654)',
    orientacion:'linear-gradient(135deg,#1a6b3a,#0d3b21)',
    deporte:'linear-gradient(135deg,#7a2e2e,#4a1515)'
  };
  var roleAccents={
    admin:'var(--gold)',profesor:'#4ade80',estudiante:'#60a5fa',padre:'#c084fc',
    enfermeria:'#34d399',secretaria:'#60a5fa',directora:'#e879f9',orientacion:'#4ade80',deporte:'#fb923c'
  };
  var bg=roleColors[role]||roleColors.admin;
  var accent=roleAccents[role]||'var(--gold)';

  var kpiCard=function(ico,val,lbl,color){
    return '<div class="kpi-card" style="background:rgba(255,255,255,0.07);border-left:4px solid '+color+';min-width:130px;"><div class="kpi-icon">'+ico+'</div><div><div class="kpi-num" style="color:'+color+';">'+val+'</div><div class="kpi-label" style="color:#ccc;">'+lbl+'</div></div></div>';
  };
  var btn=function(label,onclick,style){
    return '<button class="btn '+(style||'btn-outline')+'" style="'+(style?'':' color:white;border-color:rgba(255,255,255,0.3);')+'" onclick="'+onclick+'">'+label+'</button>';
  };

  var html='<div style="background:'+bg+';padding:32px 20px;"><div style="max-width:1100px;margin:0 auto;">';
  html+='<h3 style="font-family:\'Playfair Display\',serif;color:'+accent+';font-size:20px;margin-bottom:16px;">'+
    ({admin:'🏫',profesor:'👨‍🏫',estudiante:'🎓',padre:'👪',enfermeria:'🏥',secretaria:'🗂️',directora:'👩‍💼',orientacion:'🧭',deporte:'⚽'}[role]||'🏠')+
    ' Bienvenido/a, '+name+'</h3>';
  html+='<div class="cards-row" style="margin-bottom:12px;">';

  if(role==='admin'){
    html+=kpiCard('🎓',APP.students.length,'Estudiantes',accent);
    html+=kpiCard('📝',(APP.inscripciones||[]).length,'Inscripciones','#4ade80');
    html+=kpiCard('📅',(APP.ausencias||[]).filter(function(a){return a.status==='pending';}).length,'Ausencias Pend.','#f87171');
    html+=kpiCard('💬',(APP.mensajes||[]).length,'Mensajes','#60a5fa');
    html+=kpiCard('📋',(APP.comunicados||[]).length,'Comunicados','#fbbf24');
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Panel Admin','showPage(\'admin\')','btn-gold');
    html+=btn('+ Anuncio','openModal(\'modal-ann\')');
    html+=btn('📝 Inscripciones','showAdminSection(\'dash-inscripciones\')');
    html+=btn('📋 Comunicados','openComunicadoModal()');

  } else if(role==='profesor'){
    var pendProf=(APP.ausencias||[]).filter(function(a){return a.status==='pending';}).length;
    html+=kpiCard('🎓',APP.students.length,'Estudiantes',accent);
    html+=kpiCard('📋',(APP.notas||[]).length,'Notas',accent);
    html+=kpiCard('📅',pendProf,'Ausencias Pend.','#f87171');
    html+=kpiCard('💬',(APP.mensajes||[]).filter(function(m){return !m.leido&&m.para==='Profesor(a)';}).length,'Mensajes nuevos','#60a5fa');
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Panel','showPage(\'profesor\')','btn-gold');
    html+=btn('+ Nota','openModal(\'modal-nota\')');
    html+=btn('💬 Mensaje','openMensajeModal(\'\')');

  } else if(role==='estudiante'){
    html+=kpiCard('📋',(APP.notas||[]).filter(function(n){return n.studentId===APP.currentUser.studentId;}).length,'Mis Notas',accent);
    html+=kpiCard('📅',(APP.ausencias||[]).filter(function(a){return a.studentId===APP.currentUser.studentId;}).length,'Ausencias',accent);
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'estudiante\')','btn-gold');
    html+=btn('📢 Anuncios','showPage(\'anuncios\')');

  } else if(role==='padre'){
    html+=kpiCard('📋',(APP.notas||[]).length,'Notas del Hijo/a',accent);
    html+=kpiCard('📅',(APP.ausencias||[]).length,'Ausencias','#f87171');
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'padre\')','btn-gold');
    html+=btn('📢 Anuncios','showPage(\'anuncios\')');

  } else if(role==='enfermeria'){
    html+=kpiCard('👤',(APP.consultas||[]).length,'Consultas',accent);
    html+=kpiCard('💊',(APP.stockEnfer||[]).length,'Productos',accent);
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Panel','showPage(\'enfermeria\')','btn-gold');
    html+=btn('+ Consulta','openModal(\'modal-consulta\');populateConsultaEstSelect()');

  } else if(role==='secretaria'){
    html+=kpiCard('🎓',(APP.students||[]).length,'Estudiantes',accent);
    html+=kpiCard('📝',(APP.inscripciones||[]).length,'Inscripciones',accent);
    html+=kpiCard('👪',(APP.padres||[]).length,'Padres',accent);
    html+=kpiCard('⏳',(APP.inscripciones||[]).filter(function(i){return i.estado==='pendiente';}).length,'Pend. Aprob.','#f87171');
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'secretaria\')','btn-gold');
    html+=btn('📝 Inscripciones','showPage(\'secretaria\');showSecretariaSection(\'sec-inscripciones\',null)');
    html+=btn('🎓 Estudiantes','showPage(\'secretaria\');showSecretariaSection(\'sec-estudiantes\',null)');
    html+=btn('📥 Exportar CSV','exportStudentsCSV()');

  } else if(role==='directora'){
    html+=kpiCard('🎓',(APP.students||[]).length,'Estudiantes',accent);
    html+=kpiCard('📝',(APP.inscripciones||[]).length,'Inscripciones',accent);
    html+=kpiCard('📋',(APP.comunicados||[]).length,'Comunicados',accent);
    html+=kpiCard('👥',(APP.profesores||[]).length,'Profesores',accent);
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'directora\')','btn-gold');
    html+=btn('📈 Estadísticas','showPage(\'directora\');showDirectoraSection(\'dir-estadisticas\',null);renderDirectoraStats()');
    html+=btn('📋 Comunicado','openComunicadoModal()');

  } else if(role==='orientacion'){
    html+=kpiCard('🎓',(APP.students||[]).length,'Estudiantes',accent);
    html+=kpiCard('📂',(APP.oriCasos||[]).filter(function(c){return c.estado==='activo';}).length,'Casos Activos','#fbbf24');
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'orientacion\')','btn-gold');
    html+=btn('📂 Nuevo Caso','showPage(\'orientacion\');showOrientacionSection(\'ori-casos\',null);addOrientacionCaso()');
    html+=btn('🎓 Estudiantes','showPage(\'orientacion\');showOrientacionSection(\'ori-estudiantes\',null)');

  } else if(role==='deporte'){
    html+=kpiCard('🎓',(APP.students||[]).length,'Estudiantes',accent);
    html+=kpiCard('🏆',(APP.depEquipos||[]).length,'Equipos',accent);
    html+=kpiCard('🥇',(APP.depTorneos||[]).length,'Torneos',accent);
    html+='</div><div style="display:flex;gap:8px;flex-wrap:wrap;">';
    html+=btn('→ Mi Portal','showPage(\'deporte\')','btn-gold');
    html+=btn('🏆 Equipos','showPage(\'deporte\');showDeporteSection(\'dep-equipos\',null)');
    html+=btn('🥇 Torneos','showPage(\'deporte\');showDeporteSection(\'dep-torneos\',null)');
...
// Knowledge bases
var KB_PADRE = {
  nota:    {k:['nota','notas','promedio','calificacion','como va','rendimiento','boletin'],  r:'📋 Las notas de su hijo/a están en la pestaña **"Notas"** de su portal. Se actualizan cuando el maestro las registra.\n\n🔢 Escala de calificaciones:\n• **90-100** → Excelente 🏆\n• **80-89** → Muy Bueno ⭐\n• **70-79** → Bueno ✅\n• **65-69** → Aprobado\n• **< 65** → Necesita mejorar ⚠️\n\nEl mínimo aprobatorio es **65 puntos**.'},
  ausencia:{k:['ausencia','falta','justificar','excusa','inasistencia','falto','no fue'], r:'📅 Para justificar una ausencia:\n1. Entre a su portal → **"Excusas"**\n2. Complete el formulario con la fecha y motivo\n3. Adjunte documento médico si aplica\n4. Envíe dentro de los **3 días hábiles** siguientes\n\nSi la ausencia supera 3 días sin justificación puede afectar la evaluación del estudiante.'},
  horario: {k:['horario','hora','entrada','salida','clase','tanda'],          r:'⏰ **Horario Escolar:**\n• Entrada: **7:30 AM** (se cierra puerta 7:45)\n• Salida: **4:30 PM**\n• Lunes a Viernes\n\n📚 Los horarios de clase por materia están en la sección **"Horario"** de su portal.\n\n⚠️ Se recomienda llegar antes de las 7:30 para evitar tardanzas.'},
  mensaje: {k:['hablar','maestro','comunicar','mensaje','reunion','contactar','profesor'],   r:'💬 Tiene varias opciones para comunicarse:\n\n1. **Portal** → Sección "Mensajes" → escribir al maestro\n2. **Teléfono:** (809) 590-0771 (extensión a dirección)\n3. **WhatsApp:** al mismo número\n4. **Presencial:** lunes–viernes 8:00 AM–4:00 PM\n\nLas reuniones de padres se anuncian con anticipación en el portal.'},
  uniforme:{k:['uniforme','ropa','vestimenta','zapato'],                       r:'👔 **Uniformes del Centro:**\n\n🔵 **Diario:** Camisa blanca con logo, pantalón/falda azul marino, zapatos negros\n⚽ **Ed. Física:** Uniforme deportivo azul y blanco, tenis blancos\n🎨 **Arte/Actividades:** Pullover del centro (opcional)\n\nNo se permite el ingreso sin uniforme completo.'},
  inscripcion:{k:['inscripcion','matricula','inscribir','nuevo año','enroll'], r:'📝 **Proceso de Inscripción:**\n1. Complete el formulario online en la sección **"Inscripción"** del sitio\n2. O acérquese a secretaría con:\n   • Acta de nacimiento\n   • Certificación del centro anterior\n   • Récord de notas\n   • 2 fotos 2x2\n   • Cédula del padre/tutor\n\n⏰ Horario de atención: lunes–viernes 7:30 AM–4:30 PM'},
  reglamento:{k:['reglamento','norma','regla','disciplina','conducta'],          r:'📋 El Reglamento Interno establece:\n\n✅ **Deberes:** Puntualidad, respeto, uniforme completo, completar tareas\n❌ **Prohibido:** Uso de celular en clase, violencia, daños a la propiedad\n📱 Los celulares deben permanecer apagados durante clases\n\nPuede solicitar el reglamento completo en secretaría o descargarlo desde el portal.'},
  beca:    {k:['beca','superate','apoyo','ayuda economica','descuento'],          r:'🎓 **Apoyos y Becas Disponibles:**\n\n• **Programa Supérate** (MINERD): Para estudiantes con promedio ≥80 y necesidad económica\n• **ILAE:** Útiles escolares gratuitos\n• **Alimentación:** Desayuno escolar MINERD\n\nConsulte con el orientador del centro o visite **superate.gob.do** para más información.'},
  contacto:{k:['contacto','telefono','direccion','donde','ubicacion','llegar'],            r:'📍 **Datos de Contacto:**\n\n🗺️ Av. Charles de Gaulle, Sabana Perdida, Santo Domingo Norte\n📞 (809) 590-0771\n📱 WhatsApp: (809) 590-0771\n📧 otiliapelaezadm@gmail.com\n📘 Facebook: C.E. Otilia Peláez\n⏰ Lunes–Viernes · 7:30 AM – 4:30 PM'},
  pago:    {k:['pago','mensualidad','costo','precio','tarifa','cobro'],             r:'💰 Para información sobre aranceles y pagos:\n\n• Consulte la sección **"Tarifas"** en el sitio público\n• También puede contactar a secretaría: **(809) 590-0771**\n• Los pagos se realizan en la tesorería del centro\n\nConsulte sobre planes de pago o situaciones especiales directamente con dirección.'},
  evento:  {k:['evento','actividad','fiesta','acto','graduacion'],              r:'🎉 Los eventos y actividades escolares se anuncian en:\n• La sección **"Anuncios"** de su portal\n• El tablón de anuncios del centro\n• Comunicados en WhatsApp grupal\n\nEsté pendiente del calendario académico en su portal.'},
};
...
function fabProcess(role, msg){
  ...
  // Saludos
  if(/hola|buenos|buenas|hey|buen dia|buen tarde|que tal|como estas|saludos/.test(lower)){
    var nombre=APP.currentUser?APP.currentUser.name.split(' ')[0]:'';
    fabAddMsg(role,'bot','😊 ¡Hola'+(nombre?' '+nombre:'')+'! Soy el asistente del **C.E. Otilia Peláez**. Estoy aquí para ayudarte con cualquier consulta. ¿Sobre qué te puedo orientar?');
    fabRenderQR(role, cfg.qr);
    return;
  }
  // Agradecimientos
  if(/gracias|perfecto|excelente|ok|entendido|muy bien|genial|de nada/.test(lower)){
    fabAddMsg(role,'bot','¡Con mucho gusto! 😊 Recuerda que siempre puedes consultarme lo que necesites. Si deseas comunicarte directamente con el centro: 📞 **(809) 590-0771**.');
    fabRenderQR(role,['¿Algo más?','📍 Contacto del centro']);
    return;
  }
  // Afirmaciones cortas
  if(/^(si|sí|claro|ok|dale|aja|ajá|correcto|bien)\.?$/.test(lower.trim())){
    fabAddMsg(role,'bot','¡Perfecto! 👍 ¿En qué más te puedo ayudar?');
    fabRenderQR(role, cfg.qr);
    return;
  }
  // Cierre
  if(/adios|adiós|bye|cerrar|hasta luego|hasta pronto|chao/.test(lower)){
    fabAddMsg(role,'bot','¡Hasta luego! 👋 Que tengas un excelente día. Si necesitas ayuda en el futuro, estaré aquí.');
    setTimeout(function(){
      fabOpen[role]=false;
      var pfx=FAB_PFX[role];
      var win=document.getElementById(pfx+'-window');
  ...
  if(/quien eres|que eres|quien soy|eres humano|eres robot|eres ia|inteligencia artificial/.test(lower)){
    fabAddMsg(role,'bot','🤖 Soy el **Asistente Virtual** del Centro Educativo Otilia Peláez. Estoy entrenado para responder preguntas sobre el centro, horarios, notas, reglamento, leyes educativas y mucho más.\n\n¡Pregúntame lo que necesites!');
    fabRenderQR(role, cfg.qr);
    return;
  }
  // Preguntas sobre el centro
  if(/centro|escuela|colegio|otilia|pelaez|cuando fue|fundad|history|historia/.test(lower)){
    fabAddMsg(role,'bot','🏫 El **Centro Educativo Otilia Peláez** fue fundado en **1978**. Está ubicado en:\n\n📍 Av. Charles de Gaulle, Sabana Perdida, Santo Domingo Norte\n📞 (809) 590-0771\n⏰ Lunes–Viernes · 7:30 AM – 4:30 PM\n\nPertenecemos al **Distrito Educativo 10-02** del MINERD.');
    fabRenderQR(role,['📍 Contacto','⏰ Horario','📝 Inscripción']);
    return;
  }
  // No encontrado - respuesta inteligente
  var suggestions = cfg.qr.slice(0,3);
  fabAddMsg(role,'bot','Lo siento, no tengo información específica sobre **"'+msg+'"** en este momento. 🤔\n\nPuedo ayudarte con:\n'+suggestions.map(function(s){return '• '+s;}).join('\n')+'\n\nO puedes contactar al centro directamente:\n📞 **(809) 590-0771** · otiliapelaezadm@gmail.com');
  fabRenderQR(role, cfg.qr);
  if(!APP._botStats)APP._botStats={totalMsgs:0,unicos:0,resueltos:0,sinRespuesta:0,freq:{}};
...
if(PERSIST_KEYS.indexOf('comunicados')===-1) PERSIST_KEYS.push('comunicados');
if(PERSIST_KEYS.indexOf('notasTrimestre')===-1) PERSIST_KEYS.push('notasTrimestre');
...
function toggleSidebar(){
  ...
  if(isOpen){closeSidebar();}else{
    s.classList.add('open');s.style.display='flex';
...
function buildSidebarLinks(role){
  var h='';
  h+='<button class="sidebar-link" onclick="showPage(\'home\');closeSidebar()">🏠 Inicio</button>';
  h+='<button class="sidebar-link" onclick="showPage(\'anuncios\');closeSidebar()">📢 Anuncios</button>';
  h+='<div class="sidebar-divider"></div>';
  if(role==='admin'){
    h+='<div class="sidebar-section-title">Administración</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'admin\');closeSidebar()">⚙️ Panel Admin</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-estudiantes\');closeSidebar()">🎓 Estudiantes</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-inscripciones\');closeSidebar()">📝 Inscripciones</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-notas\');closeSidebar()">📋 Notas</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-ausencias\');closeSidebar()">📅 Ausencias</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-mensajes\');closeSidebar()">💬 Mensajes</button>';
    h+='<button class="sidebar-link" onclick="showAdminSection(\'dash-reportes\');closeSidebar()">📊 Reportes</button>';
    h+='<div class="sidebar-divider"></div>';
    h+='<div class="sidebar-section-title">Sitio Web</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'about\');closeSidebar()">📚 Sobre Nosotros</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'pagos-public\');closeSidebar()">💰 Tarifas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'reglamento-public\');closeSidebar()">📜 Reglamento</button>';
  } else if(role==='profesor'){
    h+='<div class="sidebar-section-title">Mi Panel</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'profesor\');closeSidebar()">👨‍🏫 Mi Panel</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'profesor\');showProfeSection(\'profe-notas\',null);closeSidebar()">📋 Registrar Notas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'profesor\');showProfeSection(\'profe-ausencias\',null);closeSidebar()">📅 Asistencia</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'profesor\');showProfeSection(\'profe-mensajes\',null);closeSidebar()">💬 Mensajes</button>';
    h+='<div class="sidebar-divider"></div>';
    h+='<button class="sidebar-link" onclick="showPage(\'pagos-public\');closeSidebar()">💰 Tarifas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'reglamento-public\');closeSidebar()">📜 Reglamento</button>';
  } else if(role==='estudiante'){
    h+='<div class="sidebar-section-title">Mi Portal</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'estudiante\');closeSidebar()">🎓 Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'estudiante\');showEstudianteSection(\'est-notas\');closeSidebar()">📋 Mis Notas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'estudiante\');showEstudianteSection(\'est-perfil\');closeSidebar()">👤 Mi Perfil</button>';
    h+='<div class="sidebar-divider"></div>';
    h+='<button class="sidebar-link" onclick="showPage(\'pagos-public\');closeSidebar()">💰 Tarifas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'reglamento-public\');closeSidebar()">📜 Reglamento</button>';
  } else if(role==='padre'){
    h+='<div class="sidebar-section-title">Mi Portal</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'padre\');closeSidebar()">👪 Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'padre\');showPadreSection(\'padre-notas\');closeSidebar()">📋 Ver Notas</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'padre\');showPadreSection(\'padre-inscripciones\');closeSidebar()">📝 Inscripciones</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'padre\');showPadreSection(\'padre-mensajes-section\');renderMensajes();closeSidebar()">💬 Mensajes</button>';
    h+='<div class="sidebar-divider"></div>';
    h+='<button class="sidebar-link" onclick="showPage(\'pagos-public\');closeSidebar()">💰 Tarifas</button>';
  } else if(role==='secretaria'){
    h+='<div class="sidebar-section-title">Secretaría</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'secretaria\');closeSidebar()">🗂️ Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'secretaria\');showSecretariaSection(\'sec-inscripciones\');closeSidebar()">📝 Inscripciones</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'secretaria\');showSecretariaSection(\'sec-estudiantes\');closeSidebar()">🎓 Estudiantes</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'secretaria\');showSecretariaSection(\'sec-docs\');closeSidebar()">📄 Documentos</button>';
  } else if(role==='directora'){
    h+='<div class="sidebar-section-title">Dirección</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'directora\');closeSidebar()">👩‍💼 Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'directora\');showDirectoraSection(\'dir-reportes\');closeSidebar()">📊 Reportes</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'directora\');showDirectoraSection(\'dir-personal\');closeSidebar()">👥 Personal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'directora\');showDirectoraSection(\'dir-anuncios\');closeSidebar()">📢 Anuncios</button>';
  } else if(role==='orientacion'){
    h+='<div class="sidebar-section-title">Orientación</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'orientacion\');closeSidebar()">🧭 Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'orientacion\');showOrientacionSection(\'ori-estudiantes\');closeSidebar()">🎓 Estudiantes</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'orientacion\');showOrientacionSection(\'ori-casos\');closeSidebar()">📂 Casos</button>';
  } else if(role==='deporte'){
    h+='<div class="sidebar-section-title">Educación Física</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'deporte\');closeSidebar()">⚽ Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'deporte\');showDeporteSection(\'dep-equipos\');closeSidebar()">🏆 Equipos</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'deporte\');showDeporteSection(\'dep-torneos\');closeSidebar()">🥇 Torneos</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'deporte\');showDeporteSection(\'dep-notas\');closeSidebar()">📋 Notas</button>';
  } else if(role==='enfermeria'){
    h+='<div class="sidebar-section-title">Enfermería</div>';
    h+='<button class="sidebar-link" onclick="showPage(\'enfermeria\');closeSidebar()">🏥 Mi Portal</button>';
    h+='<button class="sidebar-link" onclick="openModal(\'modal-consulta\');populateConsultaEstSelect();closeSidebar()">➕ Nueva Consulta</button>';
    h+='<button class="sidebar-link" onclick="showPage(\'enfermeria\');showEnferSection(\'enfer-registros\');renderRegistrosEnfer();closeSidebar()">📋 Historial</button>';
  }
  h+='<div class="sidebar-divider"></div>';
  h+='<button class="sidebar-link" onclick="logout()" style="color:#ff8a7a;">🚪 Cerrar Sesión</button>';
  return h;
}
...
// ══════════════════════════════════════════════
function buildPortalButtons(role){
  var el=document.getElementById('portal-buttons-row');if(!el)return;
  var btnStyle='background:linear-gradient(135deg,var(--navy),var(--blue));color:white;border:none;border-radius:14px;padding:18px 22px;cursor:pointer;font-family:\'Nunito\',sans-serif;font-weight:700;font-size:13px;text-align:center;min-width:120px;transition:all .2s;box-shadow:0 4px 14px rgba(0,0,0,0.15);';
  var cards=[];
  function card(label,fn){return '<button style="'+btnStyle+'" onmouseover="this.style.transform=\'translateY(-3px)\';this.style.boxShadow=\'0 8px 20px rgba(0,0,0,0.25)\'" onmouseout="this.style.transform=\'\';this.style.boxShadow=\'0 4px 14px rgba(0,0,0,0.15)\'" onclick="'+fn+'">'+label+'</button>';}
  if(role==='admin'){
    cards.push(card('⚙️<br>Panel Admin',"showPage('admin')"));
    cards.push(card('🎓<br>Estudiantes',"showAdminSection('dash-estudiantes')"));
    cards.push(card('📝<br>Inscripciones',"showAdminSection('dash-inscripciones')"));
    cards.push(card('📋<br>Notas',"showAdminSection('dash-notas')"));
    cards.push(card('📢<br>Anuncios',"showAdminSection('dash-anuncios')"));
    cards.push(card('💬<br>Mensajes',"showAdminSection('dash-mensajes')"));
  } else if(role==='profesor'){
    cards.push(card('👨‍🏫<br>Mi Panel',"showPage('profesor')"));
    cards.push(card('📋<br>Registrar Notas',"showPage('profesor');showProfeSection('profe-notas',null)"));
    cards.push(card('📅<br>Tomar Asistencia',"showPage('profesor');showProfeSection('profe-ausencias',null)"));
    cards.push(card('💬<br>Mensajes',"showPage('profesor');showProfeSection('profe-mensajes',null)"));
  } else if(role==='estudiante'){
    cards.push(card('🎓<br>Mi Portal',"showPage('estudiante')"));
    cards.push(card('📋<br>Mis Notas',"showPage('estudiante');showEstudianteSection('est-notas')"));
    cards.push(card('📢<br>Anuncios',"showPage('anuncios')"));
  } else if(role==='padre'){
    cards.push(card('👪<br>Mi Portal',"showPage('padre')"));
    cards.push(card('📋<br>Ver Notas',"showPage('padre');showPadreSection('padre-notas')"));
    cards.push(card('💬<br>Mensajes',"showPage('padre');showPadreSection('padre-mensajes-section');renderMensajes()"));
    cards.push(card('📝<br>Inscripciones',"showPage('padre');showPadreSection('padre-inscripciones')"));
  } else if(role==='secretaria'){
    cards.push(card('🗂️<br>Portal Secretaría',"showPage('secretaria')"));
    cards.push(card('📝<br>Inscripciones',"showPage('secretaria');showSecretariaSection('sec-inscripciones')"));
    cards.push(card('🎓<br>Estudiantes',"showPage('secretaria');showSecretariaSection('sec-estudiantes')"));
    cards.push(card('📢<br>Anuncios',"showPage('anuncios')"));
  } else if(role==='directora'){
    cards.push(card('👩‍💼<br>Portal Directora',"showPage('directora')"));
    cards.push(card('📊<br>Reportes',"showPage('directora');showDirectoraSection('dir-reportes')"));
    cards.push(card('📢<br>Anuncios',"showPage('anuncios')"));
    cards.push(card('👥<br>Gestión',"showPage('directora');showDirectoraSection('dir-personal')"));
  } else if(role==='orientacion'){
    cards.push(card('🧭<br>Portal Orientación',"showPage('orientacion')"));
    cards.push(card('🎓<br>Estudiantes',"showPage('orientacion');showOrientacionSection('ori-estudiantes')"));
    cards.push(card('📋<br>Informes',"showPage('orientacion');showOrientacionSection('ori-informes')"));
  } else if(role==='deporte'){
    cards.push(card('⚽<br>Portal Deportes',"showPage('deporte')"));
    cards.push(card('📅<br>Horarios',"showPage('deporte');showDeporteSection('dep-horarios')"));
    cards.push(card('📢<br>Anuncios',"showPage('anuncios')"));
  } else if(role==='enfermeria'){
    cards.push(card('🏥<br>Portal Enfermería',"showPage('enfermeria')"));
    cards.push(card('➕<br>Nueva Consulta',"openModal('modal-consulta');populateConsultaEstSelect()"));
    cards.push(card('📋<br>Historial',"showPage('enfermeria');showEnferSection('enfer-registros');renderRegistrosEnfer()"));
  }
  el.innerHTML=cards.join('');
}

// ══════════════════════════════════════════════
//  SECRETARÍA PORTAL FUNCTIONS
// ══════════════════════════════════════════════
function showSecretariaSection(id,btn){
  document.querySelectorAll('#page-secretaria .dash-section').forEach(s=>s.classList.remove('active'));
...
function renderSecInscripciones(){
  ...
  if(!list.length){el.innerHTML='<p style="color:#888;font-size:13px;">No hay inscripciones.</p>';return;}
  el.innerHTML='<table class="data-table"><thead><tr><th>Nombre</th><th>Grado</th><th>Fecha</th><th>Estado</th></tr></thead><tbody>'+
    list.map(i=>'<tr><td>'+i.nombre+' '+i.apellido+'</td><td>'+(i.grado||'—')+'</td><td>'+(i.fecha||'—')+'</td><td><span class="badge badge-'+(i.estado==='aprobada'?'approved':i.estado==='rechazada'?'rejected':'pending')+'">'+(i.estado||'pendiente')+'</span></td></tr>').join('')+
    '</tbody></table>';
}
function renderSecEstudiantes(){
  var el=document.getElementById('sec-estudiantes-table');if(!el)return;
  var list=APP.students||[];
  if(!list.length){el.innerHTML='<p style="color:#888;font-size:13px;">No hay estudiantes.</p>';return;}
  el.innerHTML='<table class="data-table"><thead><tr><th>Nombre</th><th>Grado</th><th>Email</th></tr></thead><tbody>'+
    list.map(s=>'<tr><td>'+s.nombre+' '+s.apellido+'</td><td>'+(s.grado||'—')+'</td><td>'+(s.email||'<span style="color:#e74c3c;font-size:11px;">Sin cuenta</span>')+'</td></tr>').join('')+
    '</tbody></table>';
  var kpi=document.getElementById('sec-kpi-estudiantes');if(kpi)kpi.textContent=list.length;
  var kpi2=document.getElementById('sec-kpi-inscripciones');if(kpi2)kpi2.textContent=(APP.inscripciones||[]).length;
}

// ══════════════════════════════════════════════
//  DIRECTORA PORTAL FUNCTIONS
// ══════════════════════════════════════════════
function showDirectoraSection(id,btn){
  document.querySelectorAll('#page-directora .dash-section').forEach(s=>s.classList.remove('active'));
  var el=document.getElementById(id);if(el)el.classList.add('active');
...
// ══════════════════════════════════════════════
//  ORIENTACIÓN PORTAL FUNCTIONS
// ══════════════════════════════════════════════
function showOrientacionSection(id,btn){
  document.querySelectorAll('#page-orientacion .dash-section').forEach(s=>s.classList.remove('active'));
  var el=document.getElementById(id);if(el)el.classList.add('active');
...
function addOrientacionCaso(){
  ...
  var desc=prompt('Descripción del caso:');if(!desc)return;
  if(!APP.oriCasos)APP.oriCasos=[];
  APP.oriCasos.push({estudiante:est,descripcion:desc,fecha:new Date().toLocaleDateString('es-DO'),estado:'activo'});
  persistSave();
  var el=document.getElementById('ori-casos-list');
  if(el)el.innerHTML=APP.oriCasos.map(c=>'<div class="ausencia-card"><div class="student-row"><span class="st-name">'+c.estudiante+'</span><span class="badge badge-'+(c.estado==='activo'?'pending':'approved')+'">'+c.estado+'</span></div><div class="reason">'+c.descripcion+'</div><small style="color:#aaa;">'+c.fecha+'</small></div>').join('');
  var k=document.getElementById('ori-kpi-casos');if(k)k.textContent=APP.oriCasos.filter(c=>c.estado==='activo').length;
  toast('Caso registrado','success');
}

// ══════════════════════════════════════════════
//  DEPORTES PORTAL FUNCTIONS
// ══════════════════════════════════════════════
function showDeporteSection(id,btn){
  document.querySelectorAll('#page-deporte .dash-section').forEach(s=>s.classList.remove('active'));
  var el=document.getElementById(id);if(el)el.classList.add('active');
...
function addEquipoDeporte(){
  ...
  var deporte=prompt('Deporte (fútbol, baloncesto, voleibol, etc.):');if(!deporte)return;
  if(!APP.depEquipos)APP.depEquipos=[];
  APP.depEquipos.push({nombre:nombre,deporte:deporte,fecha:new Date().toLocaleDateString('es-DO')});
  persistSave();
  renderDepEquipos();toast('Equipo creado','success');
}
function renderDepEquipos(){
  var el=document.getElementById('dep-equipos-list');if(!el)return;
  var list=APP.depEquipos||[];
  el.innerHTML=list.length?list.map(e=>'<div style="background:#f8f9fa;border-radius:10px;padding:16px;margin-bottom:10px;border:1px solid var(--border);"><strong>'+e.nombre+'</strong> <span style="background:#D1ECF1;color:#0c5460;padding:2px 8px;border-radius:8px;font-size:11px;margin-left:8px;">'+e.deporte+'</span></div>').join(''):'<p style="color:#888;font-size:13px;">No hay equipos.</p>';
}
function addTorneoDeporte(){
  var nombre=prompt('Nombre del torneo:');if(!nombre)return;
  var fecha=prompt('Fecha del torneo:');
  if(!APP.depTorneos)APP.depTorneos=[];
  APP.depTorneos.push({nombre:nombre,fecha:fecha||'Por definir',resultado:'Pendiente'});
  persistSave();
  var el=document.getElementById('dep-torneos-list');
  if(el)el.innerHTML=(APP.depTorneos||[]).map(t=>'<div style="background:#f8f9fa;border-radius:10px;padding:16px;margin-bottom:10px;border:1px solid var(--border);"><strong>'+t.nombre+'</strong><div style="font-size:12px;color:#888;margin-top:4px;">📅 '+t.fecha+' · '+t.resultado+'</div></div>').join('');
  toast('Torneo registrado','success');
}

// ══════════════════════════════════════════════
//  ADMIN STUDENT ACCOUNT MANAGEMENT
// ══════════════════════════════════════════════
function renderAdminCuentasEst(){
  var el=document.getElementById('admin-cuentas-est-list');if(!el)return;
  var list=APP.students||[];
  if(!list.length){el.innerHTML='<p style="color:#888;">No hay estudiantes registrados.</p>';return;}
  el.innerHTML='<table class="data-table"><thead><tr><th>Nombre</th><th>Grado</th><th>Email</th><th>Estado</th><th>Acción</th></tr></thead><tbody>'+
    list.map(s=>'<tr><td>'+s.nombre+' '+s.apellido+'</td><td>'+(s.grado||'—')+'</td><td>'+(s.email||'<em style="color:#888;">Sin correo</em>')+'</td>'+
      '<td>'+(s.pass?'<span class="badge badge-approved">✅ Activa</span>':'<span class="badge badge-pending">⚠️ Sin cuenta</span>')+'</td>'+
      '<td><button class="tbl-btn approve" onclick="adminCrearCuentaEstudiante(\''+s.id+'\')">🔑 '+(s.pass?'Cambiar':'Crear')+'</button></td></tr>').join('')+
    '</tbody></table>';
}
function adminCrearCuentaEstudiante(sid){
  var st=APP.students.find(s=>s.id===sid);if(!st)return;
  var email=prompt('Correo para '+st.nombre+' '+st.apellido+':',(st.email||''));
  if(!email)return;
  var pass=prompt('Contraseña (mínimo 6 caracteres):');
  if(!pass||pass.length<6){toast('Contraseña muy corta','error');return;}
  st.email=email.trim().toLowerCase();st.pass=pass;
  persistSave();
  renderAdminCuentasEst();
  toast('Cuenta creada para '+st.nombre,'success');
}
...
function applyDarkModeOnLoad(){
  var on=localStorage.getItem('darkMode');
  if(on==='1'){
...
function openComunicadoModal(){
  var t=document.getElementById('com-titulo'); if(t) t.value='';
  var c=document.getElementById('com-cuerpo'); if(c) c.value='';
  var d=document.getElementById('com-dest'); if(d) d.value='todos';
...
function renderComunicadosHome(){
  ...
  if(!el) return;
  var role=APP.currentUser?APP.currentUser.role:'';
  var canAdmin=role==='admin'||role==='directora'||role==='secretaria';
  var adminBtn=document.getElementById('comunicados-admin-btn');
...
function showComunicadosSection(){
  var s=document.getElementById('comunicados-home-section');
  if(!s) return;
  var visible=s.style.display==='block';
  s.style.display=visible?'none':'block';
  if(!visible) renderComunicadosHome();
}
...
function addSearchBar(containerId,tableId,placeholder){
  var container=document.getElementById(containerId);
  if(!container) return;
  var existing=container.querySelector('.search-bar-wrap');
  if(existing) return;
  var wrap=document.createElement('div');
  wrap.className='search-bar-wrap';
  var sid='search-'+tableId;
  wrap.innerHTML='<input id="'+sid+'" class="search-bar" type="text" placeholder="'+(placeholder||'Buscar...')+'" oninput="filterTable(\''+sid+'\',\''+tableId+'\')"/>';
...
function exportArrayCSV(data,fields,filename){
  if(!data||!data.length){toast('Sin datos','info');return;}
  var header=fields.map(function(f){return '"'+f.label+'"';}).join(',');
  var rows=data.map(function(row){
    return fields.map(function(f){return '"'+((row[f.key]||'')+('').replace(/"/g,'""'))+'"';}).join(',');
  });
  downloadCSV(header+'\n'+rows.join('\n'),filename||'export.csv');
}
...
function exportStudentsCSV(){
  exportArrayCSV(APP.students||[],[
    {key:'nombre',label:'Nombre'},{key:'apellido',label:'Apellido'},
    {key:'grado',label:'Grado'},{key:'seccion',label:'Sección'},
    {key:'email',label:'Correo'},{key:'fechaNac',label:'Fecha Nac.'}
  ],'estudiantes.csv');
}
function exportPadresCSV(){
  exportArrayCSV(APP.padres||[],[
    {key:'nombre',label:'Nombre'},{key:'apellido',label:'Apellido'},
    {key:'email',label:'Correo'},{key:'telefono',label:'Teléfono'},
    {key:'hijoNombre',label:'Hijo/a'}
  ],'padres.csv');
}
function exportInscripcionesCSV(){
  exportArrayCSV(APP.inscripciones||[],[
    {key:'nombre',label:'Nombre'},{key:'apellido',label:'Apellido'},
    {key:'grado',label:'Grado'},{key:'estado',label:'Estado'},
...
//  TRIMESTRE NOTAS
// ══════════════════════════════════════════════
function openNotaTrimestre(studentId){
  var st=APP.students.find(function(s){return s.id===studentId;});
  if(!st){toast('Estudiante no encontrado','error');return;}
  if(!APP.notasTrimestre) APP.notasTrimestre={};
  if(!APP.notasTrimestre[studentId]) APP.notasTrimestre[studentId]={};
  var materias=['Matemáticas','Español','Ciencias','Historia','Geografía','Inglés','Educación Física','Arte'];
  var trimestres=['T1','T2','T3'];
  var rows=materias.map(function(mat){
    var cols=trimestres.map(function(t){
      var val=(APP.notasTrimestre[studentId][mat]&&APP.notasTrimestre[studentId][mat][t])||'';
      return '<td><input type="number" min="0" max="100" value="'+val+'" id="nota-'+studentId+'-'+mat.replace(/ /g,'_')+'-'+t+'" style="width:60px;text-align:center;" /></td>';
    }).join('');
    return '<tr><td>'+mat+'</td>'+cols+'</tr>';
  }).join('');
  var html='<div style="padding:20px;"><h3>Notas de '+st.nombre+' '+st.apellido+'</h3><table class="data-table"><thead><tr><th>Materia</th><th>T1</th><th>T2</th><th>T3</th></tr></thead><tbody>'+rows+'</tbody></table><br><button class="btn-primary" onclick="saveNotaTrimestre(\''+studentId+'\')">💾 Guardar</button> <button class="btn-secondary" onclick="closeBoletin()">Cerrar</button></div>';
  var modal=document.getElementById('modal-boletin');
  if(!modal){
    modal=document.createElement('div');
    modal.id='modal-boletin';
  ...
  modal.innerHTML='<div style="background:#fff;border-radius:12px;max-width:600px;width:95%;max-height:90vh;overflow-y:auto;">'+html+'</div>';
  modal.style.display='flex';
}
function saveNotaTrimestre(studentId){
  var materias=['Matemáticas','Español','Ciencias','Historia','Geografía','Inglés','Educación Física','Arte'];
  var trimestres=['T1','T2','T3'];
  if(!APP.notasTrimestre) APP.notasTrimestre={};
  if(!APP.notasTrimestre[studentId]) APP.notasTrimestre[studentId]={};
  materias.forEach(function(mat){
    APP.notasTrimestre[studentId][mat]={};
    trimestres.forEach(function(t){
      var inp=document.getElementById('nota-'+studentId+'-'+mat.replace(/ /g,'_')+'-'+t);
      if(inp) APP.notasTrimestre[studentId][mat][t]=inp.value;
    });
  });
  persistSave();
...
function closeBoletin(){
  var m=document.getElementById('modal-boletin');
...
//  BOLETÍN IMPRIMIBLE
// ══════════════════════════════════════════════
function openBoletin(studentId){
  var st=APP.students.find(function(s){return s.id===studentId;});
  if(!st){toast('Estudiante no encontrado','error');return;}
  var notas=(APP.notasTrimestre&&APP.notasTrimestre[studentId])||{};
  var materias=['Matemáticas','Español','Ciencias','Historia','Geografía','Inglés','Educación Física','Arte'];
  var rows=materias.map(function(mat){
    var n=notas[mat]||{};
    var t1=n.T1||'—'; var t2=n.T2||'—'; var t3=n.T3||'—';
    var nums=[n.T1,n.T2,n.T3].filter(function(v){return v&&!isNaN(v);}).map(Number);
    var prom=nums.length?Math.round(nums.reduce(function(a,b){return a+b;},0)/nums.length):'—';
    var status=prom!=='—'?(prom>=60?'<span style="color:green;">✅ Aprobado</span>':'<span style="color:red;">❌ Reprobado</span>'):'';
    return '<tr><td>'+mat+'</td><td>'+t1+'</td><td>'+t2+'</td><td>'+t3+'</td><td><strong>'+prom+'</strong></td><td>'+status+'</td></tr>';
  }).join('');
  var html='<div class="boletin-container"><div class="boletin-header"><h2>Centro Educativo Otilia Peláez</h2><h3>Boletín de Calificaciones</h3><div class="boletin-info"><span><strong>Estudiante:</strong> '+st.nombre+' '+st.apellido+'</span><span><strong>Grado:</strong> '+(st.grado||'—')+'</span><span><strong>Sección:</strong> '+(st.seccion||'—')+'</span><span><strong>Año:</strong> '+new Date().getFullYear()+'</span></div></div><table class="boletin-tabla"><thead><tr><th>Materia</th><th>T1</th><th>T2</th><th>T3</th><th>Promedio</th><th>Estado</th></tr></thead><tbody>'+rows+'</tbody></table><div class="boletin-footer"><p>Generado el '+new Date().toLocaleDateString('es-MX')+'</p></div></div>';
  var modal=document.getElementById('modal-boletin');
  if(!modal){
    modal=document.createElement('div');
    modal.id='modal-boletin';
  ...
  modal.innerHTML='<div style="background:#fff;border-radius:12px;max-width:700px;width:95%;max-height:90vh;overflow-y:auto;padding:20px;">'+html+'<br><button class="print-btn" onclick="window.print()">🖨️ Imprimir</button> <button class="btn-secondary" onclick="closeBoletin()">Cerrar</button></div>';
  modal.style.display='flex';
}
...
function checkBirthdays(){
  ...
  (APP.students||[]).forEach(function(s){
    if(s.fechaNac){
      var parts=s.fechaNac.split('-');
      if(parts.length>=3){
        var bMMDD=parts[1]+'-'+parts[2];
        if(bMMDD===todayMMDD) cumples.push(s.nombre+' '+s.apellido);
      }
    }
  });
  ...
  if(banner&&msg){
    if(cumples.length){
      msg.textContent='¡Feliz cumpleaños a: '+cumples.join(', ')+'! 🎉';
...
//  DIRECTORA STATS
// ══════════════════════════════════════════════
function renderDirectoraStats(){
  var students=APP.students||[];
  var padres=APP.padres||[];
  var insc=APP.inscripciones||[];
  var comunicados=APP.comunicados||[];
  var totalEst=students.length;
  var totalPadres=padres.length;
  var aprobadas=insc.filter(function(i){return i.estado==='aprobada';}).length;
  var pendientes=insc.filter(function(i){return i.estado==='pendiente';}).length;
  var conCuenta=students.filter(function(s){return s.pass;}).length;
  var sinCuenta=totalEst-conCuenta;
  var gradoMap={};
  students.forEach(function(s){
    var g=s.grado||'Sin grado';
    gradoMap[g]=(gradoMap[g]||0)+1;
  });
  var gradoRows=Object.entries(gradoMap).map(function(e){return '<tr><td>'+e[0]+'</td><td>'+e[1]+'</td></tr>';}).join('');
  // Update top KPI cards
  var kpiEst=document.getElementById('dir-kpi-est');
  var kpiInc=document.getElementById('dir-kpi-inc');
  if(kpiEst) kpiEst.textContent=totalEst;
  if(kpiInc) kpiInc.textContent=insc.length;
  // Fill stats section
  var el=document.getElementById('dir-stats-content');
  if(!el) return;
  el.innerHTML=
    '<div class="kpi-card"><div class="kpi-icon">👪</div><div><div class="kpi-num">'+totalPadres+'</div><div class="kpi-label">Padres/Tutores</div></div></div>'+
    '<div class="kpi-card"><div class="kpi-icon">✅</div><div><div class="kpi-num">'+aprobadas+'</div><div class="kpi-label">Insc. Aprobadas</div></div></div>'+
    '<div class="kpi-card"><div class="kpi-icon">⏳</div><div><div class="kpi-num">'+pendientes+'</div><div class="kpi-label">Insc. Pendientes</div></div></div>'+
    '<div class="kpi-card"><div class="kpi-icon">🔑</div><div><div class="kpi-num">'+conCuenta+'</div><div class="kpi-label">Con Cuenta</div></div></div>'+
    '<div class="kpi-card"><div class="kpi-icon">⚠️</div><div><div class="kpi-num">'+sinCuenta+'</div><div class="kpi-label">Sin Cuenta</div></div></div>'+
    '<div class="kpi-card"><div class="kpi-icon">📋</div><div><div class="kpi-num">'+comunicados.length+'</div><div class="kpi-label">Comunicados</div></div></div>'+
    '<div style="grid-column:1/-1;"><h4 style="margin:16px 0 8px;">Estudiantes por Grado</h4>'+
    '<table class="data-table" style="max-width:400px;"><thead><tr><th>Grado</th><th>Cantidad</th></tr></thead><tbody>'+gradoRows+'</tbody></table>'+
    '<div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;">'+
    '<button class="btn-primary" onclick="exportStudentsCSV()">📥 Exportar Estudiantes CSV</button> '+
    '<button class="btn-secondary" onclick="exportPadresCSV()">📥 Exportar Padres CSV</button>'+
    '</div></div>';
}

// ══════════════════════════════════════════════
//  SECRETARIA KPI HELPER
// ══════════════════════════════════════════════
function renderSecretariaKPIs(){
  var totalEst=(APP.students||[]).length;
  var totalPadres=(APP.padres||[]).length;
  var totalInsc=(APP.inscripciones||[]).length;
  var e1=document.getElementById('sec-kpi-estudiantes');
  var e2=document.getElementById('sec-kpi-inscripciones');
  var e3=document.getElementById('sec-kpi-padres');
  if(e1) e1.textContent=totalEst;
  if(e2) e2.textContent=totalInsc;
  if(e3) e3.textContent=totalPadres;
}
// Portal navigation helpers (navigate to portal page then show tab)
function goToSecretariaPortal(tabId){
  showPage('secretaria');
  setTimeout(function(){ showSecretariaSection(tabId||'sec-inicio',null); renderSecretariaKPIs(); },80);
}
function goToDirectoraPortal(tabId){
  showPage('directora');
  setTimeout(function(){ showDirectoraSection(tabId||'dir-inicio',null); renderDirectoraStats(); },80);
}

// ══════════════════════════════════════════════
//  PATCH LOGIN TO TRIGGER POST-LOGIN ACTIONS
// ══════════════════════════════════════════════
